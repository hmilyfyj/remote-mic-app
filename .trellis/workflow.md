...
执行前读取根目录 AGENTS.md、TRELLIS.md 与 spec/guides/index.md（spec 路径相对 .trellis）。仓库约束和当前用户授权优先于下面的通用示例。
开发委托已授权时可直接创建任务并自审规划。Codex 默认 inline，分派代理需遵守当前会话授权。
任务/日志仅保留本地执行记录；详细研究和产品计划遵循 AGENTS.md 的私有仓库规则。session_auto_commit=false，提交前按授权范围审查文件。
验证命令以 spec/backend/quality-guidelines.md 与 spec/frontend/quality-guidelines.md 为准。
...
# PR creation follows BRANCH_MANAGEMENT.md and the current session tooling.
```
