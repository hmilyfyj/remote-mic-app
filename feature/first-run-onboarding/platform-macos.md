...
- `REMOTE_MIC_LOCAL_ONLY=1` 将支持范围限定为实体遥控器，控制来源页只显示“有遥控器”。
- 进入尚未完成的旧手机/Web 引导时，返回控制来源选择；保留语音工具、音频设备与按键设置。
- 实体连接、真实音频和最终文字上屏的完成门槛保持一致；设置页隐藏手机、手表与 Web 连接区。
- 此构建只验收实体路径；完整构建继续按三条控制来源执行回归。
